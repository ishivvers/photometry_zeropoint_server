...
def compare_to_SDSS( field_center, field_width, clip=True, colors=COLORS ):
    ...
    for i,filt in enumerate(['u','g','r','i','z']):
        ...
        for iii in range(len(dbins)):
            medians.append( np.median(np.abs(err[inbin==iii])) )
        plt.scatter( dbins[1:]-delt_bin/2, medians[1:], color=ccc, label=filt, s=50, alpha=.9 )
...
def web_plots_SDSS( size=1800., coords=COORDS ):
    ...
    for ifield,field_center in enumerate(coords):
        ...
        for i,band in enumerate(['u','g','r','i','z']):
            ...
            dmedians = []
            for iii in range(len(dbins)):
                dmedians.append( np.median(np.abs(err[inbin==iii])) )
            med_errs[band].append(dmedians[1:])
            plt.scatter( dbins[1:]-delt_bin/2, dmedians[1:], label="(%.2f, %.2f)" %(field_center[0], field_center[1]),\
                                                     s=50, alpha=.9, c=ccc )
...
def compare_to_UKIDSS( field_center, field_size, fn='data/ukidss_all.csv', clip=True, ignore_sdss=True ):
    ...
    for iii in range(len(dbins)):
        medians.append( np.median(np.abs(err[inbin==iii])) )
    plt.scatter( dbins[1:]-delt_bin/2, medians[1:], s=50, alpha=.9 )
...
def web_plots_UKIDSS( size=1800., ignore_sdss=True, coords=COORDS ):
    ...
    for i_field,field_center in enumerate(coords):
        ...
        for iii in range(len(dbins)):
            dmedians.append( np.median(np.abs(err[inbin==iii])) )
        med_errs.append(dmedians[1:])
        plt.scatter( dbins[1:]-delt_bin/2, dmedians[1:], c=ccc, label="(%.2f, %.2f)" %(field_center[0], field_center[1]), s=50, alpha=.9 )
...
def compare_to_APASS( field_center, field_size, fn='data/apass_all.csv', ignore_sdss=True, clip=True ):
    ...
    for i,filt in enumerate(['g','r','i','B','V']):
        ...
        plt.scatter( merr, np.abs(err), color=ccc, alpha=.5, marker='+')
...
def web_plots_APASS( size=1800, ignore_sdss=True, coords=COORDS ):
    ...
    for i_field,field_center in enumerate(coords):
        ...
        for i,filt in enumerate(['g','r','i','B','V']):
            ...
            for iii in range(len(dbins)):
                dmedians.append( np.median(np.abs(err[inbin==iii])) )
            med_errs[filt].append(dmedians[1:])
            plt.scatter( dbins[1:]-delt_bin/2, dmedians[1:], color=ccc, s=50, alpha=.9,\
                        label="(%.2f, %.2f)" %(coords[i_field][0], coords[i_field][1]) )
