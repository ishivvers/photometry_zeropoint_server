...
def catalog( field_center, field_width, object_coords=None, savefile=None, max_size=1800., return_models=False):
    ...
    if savefile:
        ...
        for i,row in enumerate(final_seds):
            row_txt = " ".join(map(lambda x: "%.6f"%x, object_coords[i]))+" "+ \
                          " ".join(map(lambda x: "%.3f"%x, row))+" "+ \
                          " ".join(map(lambda x: "%.4f"%x, errors[i]))+" "+ \
                          str(modes[i])+"\n"
            fff.write( row_txt )
...
def calc_zeropoint( input_coords, catalog_coords, input_mags, catalog_mags, clip=True, sig_clip=3., max_iter=5, convergence=.02 ):
    '''
...
def zeropoint( input_file, band, output_file=None ):
    ...
    catalog_coords, catalog_seds = catalog( field_center, field_width, object_coords=input_coords, savefile=output_file )
    
    # identify which band this is for and calculate the zeropoint
    cat_index = FILTER_PARAMS[band.lower()][2]
    catalog_mags = catalog_seds[:, cat_index ]
    zp, mad = calc_zeropoint( input_coords, catalog_coords, input_mags, catalog_mags )
    return zp, mad
